    ...
    private void fitCOD_multinomial(Solver s) {
      ...
      do {
        ...
        for (int c = 0; c < _nclass; ++c) {
          ...
          if (_state.l1pen() == 0 && _betaConstraintsOff) { // disable line search with beta constraints on
            if (_state.ginfoNull())
      ...
      try {
        while (true) {
          ...
          if (!_state._lsNeeded && (Double.isNaN(gram.likelihood) || _state.objective(gram.beta, gram.likelihood) >
                  _state.objective() + _parms._objective_epsilon) && !_checkPointFirstIter && _betaConstraintsOff) {
