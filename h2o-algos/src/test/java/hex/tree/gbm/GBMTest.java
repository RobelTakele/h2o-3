...
//      parms._tree_method = GBMModel.GBMParameters.TreeMethod.exact;
  ...
  static void compareModelHistos(List<HistogramCollectingMonitor> histos, int histosPerModel) {
    ...
    for (int i = 0; i < histos.size() - histosPerModel; i++) {
      ...
      HistogramCollectingMonitor histo_comp = histos.get(i + histosPerModel);
      for (int h = 0; h < histo._hcs.length; h++) {
        for (int c = 0; c < histo._hcs[h].length; c++) {
          assertEquals(histo._tree, histo_comp._tree);
          assertEquals(histo._k, histo_comp._k);
          assertEquals(histo._leaf, histo_comp._leaf);
          assertArrayEquals(
                  "Vals for histogram " + histo._hcs[h][c]._name + " should match, h=" + h + ",c" + c + ",i" + i,
                  histo._hcs[h][c].getRawVals(),
                  histo_comp._hcs[h][c].getRawVals(),
                  0
          );
        }
      }
